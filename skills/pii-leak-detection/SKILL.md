...
2. **Seed** — generate one **canary persona per flow** with `scripts/canary_persona.py` (unique-after-normalization name / email / E.164 phone / address / account id). Provision **one** `PII-CANARY` inbox for the account (reuse if it exists — never create a second) and use sub-addressing per flow; the playbook has the guardrail.

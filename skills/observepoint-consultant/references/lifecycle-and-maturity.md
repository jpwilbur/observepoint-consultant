...
> v0.5.0 follow-up. Treat the timelines here as illustrative defaults, not commitments.
...
**Common stuck-at-this-stage patterns.** "Alerts but noisy" — too many alerts, thresholds too tight, or alerts on Rules that flap, so the team learns to ignore them and the signal drowns. And the strategic ceiling: "a healthy program but no executive sponsor" — the operation runs well at the practitioner level but has no business owner, which is the single biggest renewal risk because nobody above the analyst can articulate the program's value when budget season comes. The account health and strategy reference treats this stuck pattern as a first-class renewal signal.
...
**Year 1 — maturity transition, QBR cadence, renewal-prep groundwork.** By the one-year mark the program should be at **run** and reaching toward **fly**: tuned alerts, a named owner, a recurring quarterly business review, and an evidence trail that demonstrates value. Year 1 is also where renewal preparation begins in earnest — not as a sales motion but as the natural output of a program that's been accumulating evidence of its own worth. The ROI and renewal reference covers how to frame that value; the account health and strategy reference covers reading whether the account is actually on track for it.

...
**Year 1 — maturity transition, QBR cadence, renewal-prep groundwork.** By the one-year mark the program should be at **run** and reaching toward **fly**: tuned alerts, a named owner, a recurring quarterly business review, and an evidence trail that demonstrates value. Year 1 is also where renewal preparation begins in earnest — not as a sales motion but as the natural output of a program that's been accumulating evidence of its own worth. `references/roi-and-renewal-framing.md` covers how to frame that value; `references/account-health-and-strategy.md` covers reading whether the account is actually on track for it.

...
- **What the Rule should assert** — the `WHEN … EXPECT …` logic for a GA4 purchase event, a consent leak, a tag-presence check → the relevant domain skill (analytics-validation, privacy-compliance, tags). They decide the contract; I show how to POST it to `/v3/rules`.
...
- **Decide what to validate.** I show how to POST a Rule and gate a build; the analytics-validation, privacy-compliance, and tags skills decide what the Rule should assert and what "passing" means.

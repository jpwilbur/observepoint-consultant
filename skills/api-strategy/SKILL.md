...
- **What the Rule should assert** — the `WHEN … EXPECT …` logic for a GA4 purchase event, a consent leak, a tag-presence check → the relevant domain skill (tag-and-analytics-quality, privacy-compliance). They decide the contract; I show how to POST it to `/v3/rules`.
- **Which regulation or accessibility standard the audit evidences** — → the `privacy-compliance` and `accessibility` skills. I run the audit on a schedule; they say what "passing" means.
...
- **Decide what to validate.** I show how to POST a Rule and gate a build; the tag-and-analytics-quality and privacy-compliance skills decide what the Rule should assert and what "passing" means.

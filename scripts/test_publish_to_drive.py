...
def test_publish_places_new_and_archives_old(tmp_path):
    ...
    (drive / "observepoint-consultant-0.15.0.plugin").write_bytes(b"old")
    art = _make_artifact(tmp_path, "observepoint-consultant-0.16.0.plugin")
    result = ptd.publish(art, drive)
    assert result["published"] is True
    assert result["archived"] == ["observepoint-consultant-0.15.0.plugin"]
    assert [p.name for p in drive.glob("*.plugin")] == ["observepoint-consultant-0.16.0.plugin"]
    assert (drive / "Old Versions" / "observepoint-consultant-0.15.0.plugin").is_file()
...
def test_publish_first_release_no_archive(tmp_path):
    ...
    assert (drive / "observepoint-consultant-0.16.0.plugin").is_file()
...
def test_publish_self_heals_to_one_current(tmp_path):
    drive = _drive(tmp_path)
    (drive / "observepoint-consultant-0.15.0.plugin").write_bytes(b"old")
    (drive / "observepoint-consultant-0.16.0.plugin").write_bytes(b"stale-current")
    art = _make_artifact(tmp_path, "observepoint-consultant-0.16.0.plugin")
    ptd.publish(art, drive)
    assert [p.name for p in drive.glob("*.plugin")] == ["observepoint-consultant-0.16.0.plugin"]
    assert (drive / "Old Versions" / "observepoint-consultant-0.15.0.plugin").is_file()
    assert (drive / "observepoint-consultant-0.16.0.plugin").read_bytes() == b"PK\x03\x04 fake zip"
...
def test_dry_run_touches_nothing(tmp_path):
    ...
    assert result["published"] is True
    assert (drive / "observepoint-consultant-0.15.0.plugin").is_file()
    assert not (drive / "observepoint-consultant-0.16.0.plugin").exists()
    assert not (drive / "Old Versions").exists()
...
def test_current_published_version(tmp_path):
    ...
    (drive / "observepoint-consultant-0.16.1.plugin").write_bytes(b"x")
...
def test_main_publishes(tmp_path):
    drive = tmp_path / "ObservePoint Revenue"
    drive.mkdir()
    art = tmp_path / "observepoint-consultant-0.16.0.plugin"

...
# This repo IS the plugin at its root, so PLUGIN_SUBDIR defaults to empty (the
# marketplace.json that lives alongside plugin.json is stripped from the bundle below).

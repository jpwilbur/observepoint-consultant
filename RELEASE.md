...
`Solutions Consulting / Claude / Plugins / ObservePoint Consultant /`
...
3. The hook detects the new version and runs `./scripts/release.sh`, which:
   - builds `dist/observepoint-consultant-<version>.plugin` (gitignored, local only),
     gated on a metadata check (description ≤ 500 chars, no XML tags in skill
     descriptions — the Cowork upload rules) **and** `claude plugin validate`;
...
- **Drive not found / ambiguous:** set the target folder explicitly:
  `OP_PLUGIN_DRIVE_DIR="/path/to/Plugins/ObservePoint Consultant" ./scripts/release.sh`
- **Build only (no publish):** `bash scripts/build_plugin.sh`
- **Check upload rules only:** `python3 scripts/check_plugin_meta.py .`
